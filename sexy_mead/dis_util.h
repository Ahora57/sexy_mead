...
namespace dis
{
    ...
    auto get_len(char* runtime_address) -> ZyanStatus
    {
    ...
    ZyanU64 get_absolute_address(ZydisDisassembledInstruction* instruction, CHAR* addr)
    {

        ZyanU64 destination = 0ULL;
        ZyanU64 runtime_address = reinterpret_cast<ZyanU64>(addr);
        for (ZyanU8 i = NULL; i < instruction->info.operand_count; i++)
        {
            if ((instruction->operands[i].type == ZYDIS_OPERAND_TYPE_IMMEDIATE && instruction->operands[i].imm.is_relative == ZYAN_TRUE) || instruction->operands[i].type == ZYDIS_OPERAND_TYPE_MEMORY)
            {
                ZydisCalcAbsoluteAddress(&instruction->info, &instruction->operands[i], runtime_address, &destination);
                break;
            }

            if (instruction->operands[i].type == ZYDIS_OPERAND_TYPE_IMMEDIATE && instruction->operands[i].imm.is_relative == ZYAN_FALSE)
            {
